...
fn main() {
    ...
    let catalog_path = manifest_dir.join("data/chrome-145-windows-v1.json");
    ...
    let output = json!({
        ...
        "webgpu": selected_row(&catalog["components"]["webgpu"], webgpu_id, "components.webgpu")
    });
    let out = PathBuf::from(env::var_os("OUT_DIR").unwrap()).join("chrome-145-windows-v1.default.json");
    fs::write(out, serde_json::to_vec(&output).expect("serialize fixed profile"))
        .expect("write fixed profile");
    let compressed_path = PathBuf::from(env::var_os("OUT_DIR").unwrap()).join("chrome-145-windows-v1.json.gz");
